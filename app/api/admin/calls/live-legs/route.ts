...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (Array.isArray(ids) && ids.length > 0) {
      const wanted = ids.filter((i): i is string => typeof i === 'string' && i.length > 0)
      if (wanted.length === 0) {
        return NextResponse.json(
          { success: false, error: 'callControlIds contained no usable ids' },
          { status: 400 }
        )
      }
    ...
    if (typeof olderThan === 'number' && Number.isFinite(olderThan) && olderThan >= 0) {
