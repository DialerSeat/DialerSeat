// =============================================================================
// NANP AREA CODE -> STATE
// =============================================================================
// The map already knows where the AGENT is, from their heartbeat. This is the
// other half: where the call went. A dialer's map that only plots its own
// operators is a map of one office; plotting the destination is what makes it
// a picture of the business.
//
// Area code is the only geography a phone number carries, and it is honest
// about being approximate — mobile numbers keep their original area code
// forever, so this says "this number was issued in Ohio", never "this person
// is in Ohio". Good enough to colour a map, not good enough to act on, which
// is why nothing downstream does anything with it except draw.
//
// US and Canada only. The rest of the NANP (Caribbean) is deliberately absent:
// those codes map to countries this list would have to model differently, and
// an unknown code draws nothing rather than drawing something wrong.
// =============================================================================
...
export const AREA_CODE_STATE: Record<string, string> = {
  ...
  '530': 'CA', '559': 'CA', '562': 'CA', '619': 'CA', '626': 'CA', '627': 'CA',
  ...
  '728': 'FL', '754': 'FL', '772': 'FL', '786': 'FL', '813': 'FL', '850': 'FL',
...
/** Toll-free and service codes carry no geography at all — never guess. */
const NON_GEOGRAPHIC = new Set([
  '800', '833', '844', '855', '866', '877', '888', '822', '880', '881', '882',
  '900', '976', '211', '311', '411', '511', '611', '711', '811', '911',
])

/**
 * The state a number was issued in, or null.
 *
 * Accepts anything the calls table holds — +1XXXXXXXXXX, 10 digits, or 11 with
 * a leading 1 — because that column has been written by several code paths
 * over time and a strict parser would silently drop half the feed.
 */
export function stateForNumber(raw: string | null | undefined): string | null {
  if (!raw) return null
  ...
  if (NON_GEOGRAPHIC.has(npa)) return null
